...
- \`[нач_органа]\` — должность лица с госоргана
...
- \`[обращение]\` — способ обращения
- \`[способ обезвреживания]\` — способ обезвреживания
- \`[степень]\` — степень опасности отхода
- \`[Периодичность вывоза]\` — периодичность вывоза
- \`[Обоснование периодичности]\` — обоснование периодичности
- \`[СХЕМА]\` — изображение карты-схемы
- \`[участок]\` — наименование участка / здания
- \`[номер_источника]\` — номер источника образования
- \`[номер_отхода]\` — номер отхода
- \`[код]\` — код отхода
- \`[отход]\` — наименование отхода
- \`[класс]\` — класс опасности
- \`[допустимое_количество]\` — допустимое количество накопления
...
- \`[название_организации_полное]\` — полное название
- \`[юридический_адрес]\` — юридический адрес
- \`[телефон]\` — телефон
- \`[УНП]\` — учётный номер плательщика
- \`[орган_регистрации]\` — орган регистрации
- \`[листы]\` — количество страниц инструкции
- \`[должность_руководителя]\` — должность руководителя
- \`[ФИО_руководителя]\` — инициалы и фамилия
- \`[перечень]\` — перечень документов
`,
    parentId: 'templates-instruction',
    order: 1,
    isTemplate: true,
