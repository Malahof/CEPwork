...
| `GET` | `/api/docs/versions` | Получить список последних версий JSON-снапшота |
| `GET` | `/api/docs/versions/:versionId` | Получить полный snapshot выбранной версии |
| `POST` | `/api/docs/restore/:versionId` | Восстановить проект из версии с сохранением текущего состояния в историю |
...
Документы хранятся как единый JSON-снапшот `data/docs.json`. Перед каждым успешным `POST /api/docs`
сервер сохраняет предыдущий snapshot в `data/versions/{timestamp}_{versionNum}.json`.
Хранится не более 50 последних версий; старые файлы автоматически удаляются.

В интерфейсе кнопка `История версий` открывает список snapshot-версий с датой и размером файла.
Восстановление выбранной версии перезаписывает весь проект, но перед restore текущее состояние
сохраняется как новая версия, чтобы изменения можно было вернуть.
