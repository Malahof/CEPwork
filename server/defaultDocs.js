...
`,
  ...
  },
  {
    ...
    title: 'Инструкция',
    content: `# Шаблон: Инструкция по обращению с отходами

## Переменные, которые заменяются в DOCX

- \`[название_организации]\` — сокращённое название организации
- \`[название_организации_полное]\` — полное название организации
- \`[юридический_адрес]\` — адрес регистрации
- \`[УНП]\` — учётный номер плательщика
- \`[дата_регистрации]\` — дата государственной регистрации
- \`[орган_регистрации]\` — орган, зарегистрировавший ЮЛ
- \`[вид_деятельности]\` — основной вид деятельности (ОКЭД)
- \`[Место]\` — населённый пункт из юридического адреса
- \`[должность_руководителя]\` — должность руководителя
- \`[ФИО_руководителя]\` — инициалы и фамилия руководителя
- \`[нач_органа]\` — должность лица с госоргана (Заместитель председателя / Начальник)
- \`[орган]\` — орган Минприроды в родительном падеже
- \`[фио_нач_органа]\` — инициалы и фамилия лица с госоргана
- \`[адрес]\` — места осуществления деятельности (маркированный список)
- \`[должность]\` — должность ответственного лица (повторяется)
- \`[норматив]\` — норматив образования отхода
- \`[определение]\` — способ определения фактического объёма
- \`[физ_сост]\` — физическое состояние отхода
- \`[источник]\` — источник образования отхода
- \`[способ]\` — способ хранения
- \`[размер]\` — размер площадки / объём контейнера / ёмкости
- \`[обращение]\` — способ обращения (заготовка, использование, обезвреживание, захоронение)
- \`[способ обезвреживания]\` — способ обезвреживания для собственного объекта
- \`[степень]\` — степень опасности отхода
- \`[Периодичность вывоза]\` — периодичность вывоза отходов
- \`[Обоснование периодичности]\` — обоснование периодичности вывоза
- \`[СХЕМА]\` — изображение карты-схемы
- \`[участок]\` — наименование участка / здания
- \`[номер_источника]\` — порядковый номер источника образования
- \`[номер_отхода]\` — порядковый номер отхода
- \`[код]\` — код отхода
- \`[отход]\` — наименование отхода
- \`[класс]\` — класс опасности
- \`[допустимое_количество]\` — допустимое количество накопления, т
- \`[обоснование]\` — обоснование допустимого количества
`,
  ...
  {
    id: 'template-instruction-application',
    title: 'Заявление',
    content: `# Шаблон: Заявление на согласование инструкции

## Переменные, которые заменяются в DOCX

- \`[орган]\` — орган Минприроды в родительном падеже
- \`[адрес_органа]\` — адрес органа
- \`[название_организации_полное]\` — полное название организации
- \`[юридический_адрес]\` — юридический адрес организации
- \`[телефон]\` — телефон организации
- \`[УНП]\` — учётный номер плательщика
- \`[орган_регистрации]\` — орган, зарегистрировавший ЮЛ
- \`[листы]\` — количество страниц в инструкции
- \`[должность_руководителя]\` — должность руководителя
- \`[ФИО_руководителя]\` — инициалы и фамилия руководителя
- \`[перечень]\` — перечень прилагаемых документов
`,
    parentId: 'templates-instruction',
    order: 1,
    isTemplate: true,
