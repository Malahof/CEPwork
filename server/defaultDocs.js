...
export function ensureDefaultDocsStructure(snapshot) {
  ...
  for (const page of snapshot.pages ?? []) {
    if (!legacySampleTemplatePageIds.has(page.id)) pagesById.set(page.id, { ...page });
  }

  for (const page of pageDefinitions) {
    const existing = pagesById.get(page.id);
    pagesById.set(page.id, existing ? { ...page, ...existing, title: page.title, parentId: page.parentId, order: page.order, isTemplate: page.isTemplate } : withTimestamps(page));
  }
  ...
  const activePageId = legacySampleTemplatePageIds.has(snapshot.activePageId ?? '') ? 'welcome' : snapshot.activePageId ?? 'welcome';

  return { pages, folders, activePageId };
}
